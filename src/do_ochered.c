...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/30 17:58:45 by idunaver          #+#    #+#             */
/*   Updated: 2019/09/30 18:08:26 by idunaver         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_to_ochered2(t_way **ochered,
t_connection *temp, int line, t_v *node)
{
	t_way	*ochered1;
	t_way	*ochered2;
...
int		one_from_zero(t_way *ptr, t_v *node, t_way **ochered, int line)
{
	...
	temp = node->knot;
	while (temp->bonds->vizit != 1 && temp != NULL)
	{
...
void	do_ochered(t_v **hashtab, char *start,
t_way **ochered, int line, t_flag *fl)
{
	t_connection	*temp;
	t_v				*node;
	t_way			*ptr;
...
t_way	*ft_way(t_v **hashtab, char *start, char *end, t_flag *fl)
{
	...
	while (temp)
	{
		if (temp->status == 0)
		{
			do_ochered(hashtab, temp->sosed->key,
			&ochered, temp->poryadok + 1, fl);
			temp->status = 1;
		}
		if (fl->k == 1)
